...
@Component({
  ...
  styles: [`
    ...
    .hero-badge { display: inline-block; padding: 0.375rem 1rem; border-radius: 100px; background: rgba(54, 195, 254, 0.15); color: var(--primary-hover); font-size: 0.875rem; font-weight: 600; margin-bottom: 1rem; }
    .upgrade-hero h1 { margin-bottom: 0.75rem; }
    .upgrade-hero p { color: var(--text-secondary); font-size: 1.125rem; }
    .plans-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 2rem; max-width: 900px; margin: 0 auto; }
    .plan-card { background: var(--card-bg); border: 1px solid var(--border-color); border-radius: 24px; padding: 2rem; display: flex; flex-direction: column; }
    .plan-card.premium { border-color: var(--primary); box-shadow: 0 0 40px rgba(96, 89, 247, 0.12); position: relative; }
    .plan-badge-top { position: absolute; top: -14px; left: 50%; transform: translateX(-50%); background: linear-gradient(135deg, var(--primary-hover), var(--primary)); color: #fff; padding: 0.25rem 1rem; border-radius: 100px; font-size: 0.75rem; font-weight: 700; letter-spacing: 0.08em; }
    .plan-header { margin-bottom: 1.5rem; }
    .plan-price { font-size: 2.5rem; font-weight: 800; color: var(--primary-hover); margin-top: 0.5rem; }
    ...
    .plan-current { text-align: center; color: var(--text-muted); font-size: 0.875rem; padding: 0.75rem; border: 1px solid rgba(101, 146, 254, 0.15); border-radius: 12px; }
    .plan-note { text-align: center; color: var(--text-muted); font-size: 0.8125rem; margin-top: 0.75rem; }
